...
/*
 ...
 - General Gamepad controls: enable with ImGuiConfigFlags_NavEnableGamepad. See suggested mappings in imgui.hpp ImGuiNavInput_ + download PNG/PSD at http://goo.gl/9LgVZW
 ...
 - Overwrite all the sources files except for imconfig.hpp (if you have made modification to your copy of imconfig.hpp)
 ...
 - You can later customize the imconfig.hpp file to tweak some compilation time behavior, such as integrating imgui types with your own maths types.
 ...
 - Gamepad:
    ...
    - See 'enum ImGuiNavInput_' in imgui.hpp for a description of inputs. For each entry of io.NavInputs[], set the following values:
 ...
 A: Short explanation:
    ...
    Here's a simplified OpenGL example using stb_image.hpp:

        // Use stb_image.hpp to load a PNG from disk and turn it into raw RGBA pixel data:
 ...
 A: You can edit imconfig.hpp and setup the IM_VEC2_CLASS_EXTRA/IM_VEC4_CLASS_EXTRA macros to add implicit type conversions.
...
// - Change this variable to use thread local storage. You may #define GImGui in imconfig.hpp for that purpose. Future development aim to make this context pointer explicit to all calls. Also read https://github.com/ocornut/imgui/issues/586
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    // B) If you need/want meshes with more than 64K vertices, uncomment the '#define ImDrawIdx unsigned int' line in imconfig.hpp to set the index size to 4 bytes.
...
// Prefer just including imgui_internal.hpp from your code rather than using this define. If a declaration is missing from imgui_internal.h add it or request it on the github.
