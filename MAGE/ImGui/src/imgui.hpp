...
// Configuration file (edit imconfig.hpp or define IMGUI_USER_CONFIG to set your own filename)
#ifdef IMGUI_USER_CONFIG
#include IMGUI_USER_CONFIG
#endif
#if !defined(IMGUI_DISABLE_INCLUDE_IMCONFIG_HPP) || defined(IMGUI_INCLUDE_IMCONFIG_HPP)
...
// IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used for the default bindings files (imgui_impl_xxx.hpp)
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
#ifndef IM_ASSERT
#include <assert.h>
#define IM_ASSERT(_EXPR)            assert(_EXPR)                               // You can override the default assert handler by editing imconfig.hpp
#endif
...
struct ImVec2
{
    ...
    IM_VEC2_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.hpp to convert back and forth between your math types and ImVec2.
#endif
};
...
struct ImVec4
{
    ...
    IM_VEC4_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.hpp to convert back and forth between your math types and ImVec4.
#endif
};

// Dear ImGui end-user API
// (In a namespace so you can add extra functions in your own separate file. Please don't modify imgui.cpp/.hpp!)
namespace ImGui
{
...
#else
    // This is only here to keep ImGuiIO the same size, so that IMGUI_DISABLE_OBSOLETE_FUNCTIONS can exceptionally be used outside of imconfig.hpp.
...
// Vertex index (override with '#define ImDrawIdx unsigned int' inside in imconfig.hpp)
#ifndef ImDrawIdx
...
#else
// You can override the vertex format layout by defining IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT in imconfig.hpp
...
// Include imgui_user.hpp at the end of imgui.hpp (convenient for user to only explicitly include vanilla imgui.hpp)
#ifdef IMGUI_INCLUDE_IMGUI_USER_HPP
