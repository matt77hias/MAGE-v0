...
float4 PS(PSInputPositionNormalTexture input) : SV_Target {
	...
	const float3 c      = UnpackNormal(g_normal_texture.Sample(g_linear_wrap_sampler, input.tex2));
	...
	const float4 Kd = g_Kd * g_diffuse_texture.Sample(g_linear_wrap_sampler, input.tex);
	...
	const float3 Ks = g_Ks * g_specular_texture.Sample(g_linear_wrap_sampler, input.tex).xyz;
