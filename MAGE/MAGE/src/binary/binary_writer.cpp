...
namespace mage {
	...
	void BigEndianBinaryWriter::WriteToFile(const wstring &fname) {
		...
		FILE *file;
		const errno_t result_fopen_s = 
			_wfopen_s(&file, GetFilename().c_str(), L"wb");
		if (result_fopen_s) {
			throw FormattedException("%ls: could not open file.", 
				GetFilename().c_str());
		}
	...
	void BigEndianBinaryWriter::WriteCharacter(char c) {
		...
		if (result == EOF) {
			throw FormattedException("%ls: could not write to file.", 
				GetFilename().c_str());
		}
	}
