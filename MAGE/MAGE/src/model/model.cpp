...
namespace mage {
	...
	Model::Model(ID3D11Device2 *device, const string &name, const string &path)
		: Resource(name, path), m_vertex_buffer(nullptr), m_index_buffer(nullptr) {
	...
	HRESULT Model::InitializeBuffers(ID3D11Device2 *device) {

		const string &fname = GetFilename();
		vector< Vertex > vertices;
		vector< uint32_t > indices;
		const HRESULT result_load = LoadModelFromFile(fname, vertices, indices);
		if (FAILED(result_load)) {
			Warning("Model loading failed: %ld.", result_load);
			return result_load;
		}
	...
	HRESULT Model::UninitializeBuffers() {
		SAFE_RELEASE(m_vertex_buffer);
		SAFE_RELEASE(m_index_buffer);
		return S_OK;
	}

	HRESULT Model::SetupVertexBuffer(ID3D11Device2 *device, const Vertex *vertices, size_t nb_vertices) {
	...
	HRESULT Model::SetupIndexBuffer(ID3D11Device2 *device, const uint32_t *indices, size_t nb_indices) {
		// Describe the buffer resource.
		D3D11_BUFFER_DESC buffer_desc;
		ZeroMemory(&buffer_desc, sizeof(buffer_desc));
	...
	HRESULT Model::BindBuffers(ID3D11DeviceContext2 *device_context) {
