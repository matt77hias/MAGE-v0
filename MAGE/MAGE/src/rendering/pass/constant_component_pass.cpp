...
namespace mage {
	...
	ConstantComponentPass::ConstantComponentPass(
		ConstantComponentPass &&pass) noexcept = default;

	ConstantComponentPass::~ConstantComponentPass() = default;

	void ConstantComponentPass::BindFixedState() const noexcept {
	...
	void ConstantComponentPass::BindPS(RenderMode render_mode) {
		...
		case RenderMode::UVTexture: {
			...
			m_ps[static_cast< size_t >(PSIndex::UVTexture)]
				->BindShader(m_device_context);

			// Update the color buffer.
			m_color_buffer.UpdateData(m_device_context, RGBA(1.0f));
			// Bind the color buffer.
			m_color_buffer.Bind< Pipeline::PS >(m_device_context, 
												SLOT_CBUFFER_COLOR);
			// Bind the texture SRV.
			Pipeline::PS::BindSRV(m_device_context, SLOT_SRV_TEXTURE, 
								  m_uv->Get());
		...
		case RenderMode::Distance: {
			// Bind the pixel shader.
			m_ps[static_cast< size_t >(PSIndex::Distance)]
				->BindShader(m_device_context);
	...
	void XM_CALLCONV ConstantComponentPass
