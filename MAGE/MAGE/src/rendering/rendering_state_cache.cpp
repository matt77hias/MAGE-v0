...
namespace mage {
	...
	ID3D11BlendState *RenderingStateCache::GetOpaqueBlendState() {
		...
		if (!m_opaque_blend_state) {
			const HRESULT result_create = CreateOpaqueBlendState(m_device, 
				m_opaque_blend_state.ReleaseAndGetAddressOf());
	...
	ID3D11BlendState *RenderingStateCache::GetAlphaBlendState() {
		...
		if (!m_alpha_blend_state) {
			const HRESULT result_create = CreateAlphaBlendState(m_device, 
				m_alpha_blend_state.ReleaseAndGetAddressOf());
	...
	ID3D11BlendState *RenderingStateCache::GetAdditiveBlendState() {
		...
		if (!m_additive_blend_state) {
			const HRESULT result_create = CreateAdditiveBlendState(m_device, 
				m_additive_blend_state.ReleaseAndGetAddressOf());
	...
	ID3D11BlendState *RenderingStateCache::GetNonPremultipliedBlendState() {
		...
		if (!m_non_premultiplied_blend_state) {
			const HRESULT result_create = CreateNonPremultipliedBlendState(m_device, 
				m_non_premultiplied_blend_state.ReleaseAndGetAddressOf());
	...
	ID3D11BlendState *RenderingStateCache::GetAlphaToCoverageBlendState() {
		...
		if (!m_alpha_to_coverage_blend_state) {
			const HRESULT result_create = CreateAlphaToCoverageBlendState(m_device, 
				m_alpha_to_coverage_blend_state.ReleaseAndGetAddressOf());
	...
	ID3D11DepthStencilState *RenderingStateCache::GetDepthNoneDepthStencilState() {
		...
		if (!m_depth_none_depth_stencil_state) {
			const HRESULT result_create = CreateDepthNoneDepthStencilState(m_device, 
				m_depth_none_depth_stencil_state.ReleaseAndGetAddressOf());
	...
	ID3D11DepthStencilState *RenderingStateCache::GetDepthDefaultDepthStencilState() {
		...
		if (!m_depth_default_depth_stencil_state) {
			const HRESULT result_create = CreateDepthDefaultDepthStencilState(m_device, 
				m_depth_default_depth_stencil_state.ReleaseAndGetAddressOf());
	...
	ID3D11DepthStencilState *RenderingStateCache::GetDepthReadDepthStencilState() {
		...
		if (!m_depth_read_depth_stencil_state) {
			const HRESULT result_create = CreateDepthReadDepthStencilState(m_device, 
				m_depth_read_depth_stencil_state.ReleaseAndGetAddressOf());
	...
	ID3D11RasterizerState *RenderingStateCache::GetCullNoneRasterizerState() {
		...
		if (!m_cull_none_rasterizer_state) {
			const HRESULT result_create = CreateCullNoneRasterizerState(m_device, 
				m_cull_none_rasterizer_state.ReleaseAndGetAddressOf());
	...
	ID3D11RasterizerState *RenderingStateCache::GetCullClockwiseRasterizerState() {
		...
		if (!m_cull_clockwise_rasterizer_state) {
			const HRESULT result_create = CreateCullClockwiseRasterizerState(m_device, 
				m_cull_clockwise_rasterizer_state.ReleaseAndGetAddressOf());
	...
	ID3D11RasterizerState *RenderingStateCache::GetCullCounterClockwiseRasterizerState() {
		...
		if (!m_cull_counter_clockwise_rasterizer_state) {
			const HRESULT result_create = CreateCullCounterClockwiseRasterizerState(m_device, 
				m_cull_counter_clockwise_rasterizer_state.ReleaseAndGetAddressOf());
	...
	ID3D11RasterizerState *RenderingStateCache::GetWireframeRasterizerState() {
		...
		if (!m_wireframe_rasterizer_state) {
			const HRESULT result_create = CreateWireframeRasterizerState(m_device, 
				m_wireframe_rasterizer_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetPointWrapSamplerState() {
		...
		if (!m_point_wrap_sampler_state) {
			const HRESULT result_create = CreatePointWrapSamplerState(m_device, 
				m_point_wrap_sampler_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetPointClampSamplerState() {
		...
		if (!m_point_clamp_sampler_state) {
			const HRESULT result_create = CreatePointClampSamplerState(m_device, 
				m_point_clamp_sampler_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetLinearWrapSamplerState() {
		...
		if (!m_linear_wrap_sampler_state) {
			const HRESULT result_create = CreateLinearWrapSamplerState(m_device, 
				m_linear_wrap_sampler_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetLinearClampSamplerState() {
		...
		if (!m_linear_clamp_sampler_state) {
			const HRESULT result_create = CreateLinearClampSamplerState(m_device, 
				m_linear_clamp_sampler_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetAnisotropicWrapSamplerState() {
		...
		if (!m_anisotropic_wrap_sampler_state) {
			const HRESULT result_create = CreateAnisotropicWrapSamplerState(m_device, 
				m_anisotropic_wrap_sampler_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetAnisotropicClampSamplerState() {
		...
		if (!m_anisotropic_clamp_sampler_state) {
			const HRESULT result_create = CreateAnisotropicClampSamplerState(m_device, 
				m_anisotropic_clamp_sampler_state.ReleaseAndGetAddressOf());
	...
	ID3D11SamplerState *RenderingStateCache::GetPCFSamplerState() {
		const MutexLock lock(m_mutex);
		if (!m_pcf_sampler_state) {
			const HRESULT result_create = CreatePCFSamplerState(m_device, 
				m_pcf_sampler_state.ReleaseAndGetAddressOf());
			ThrowIfFailed(result_create, "PCF sampler state creation failed: %08X.", result_create);
		}
		return m_pcf_sampler_state.Get();
	}
}