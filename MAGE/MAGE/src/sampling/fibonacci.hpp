...
namespace mage {
	...
	inline void FibonacciSpiralSamplesInUnitCircle(XMFLOAT2 *samples, size_t nb_samples, 
		float shift = 1.0f, float alpha = 2.0f) {
		...
		for (size_t i = j; i <= nb_samples - np_boundary; ++i, ++j, ++samples) {
			...
			*samples = XMFLOAT2(r * cos(phi), r * sin(phi));
		}
	...
	inline void FibonacciSpiralSamplesOnUnitSphere(XMFLOAT3 *samples, size_t nb_samples, 
		float shift = 1.0f) {
		...
		for (size_t i = 0; i < nb_samples; ++i, ++samples) {
			...
			*samples = XMFLOAT3(cos(phi) * sin_theta, cos_theta, sin(phi) * sin_theta);
		}
	}

	inline void FibonacciSpiralSamplesOnUnitHemisphere(XMFLOAT3 *samples, size_t nb_samples, 
		float shift = 1.0f, bool positive = true) {
	...
	inline void FibonacciSpiralCosineWeightedSamplesOnUnitHemisphere(XMFLOAT3 *samples, size_t nb_samples,
		float shift = 1.0f) {
