...
namespace mage {
	...
	inline float RadicalInverse(size_t index, float base) noexcept {
		float result = 0.0f;
		float denom  = 1.0f;
	...
	inline float VanderCorput(size_t index) noexcept {
		return RadicalInverse(index, 2.0f);
	}

	inline void Halton(size_t index, float *sample, size_t nb_dims) noexcept {
		Assert(nb_dims < _countof(g_primes));

		for (size_t i = 0; i < nb_dims; ++i, ++sample) {
			*sample = RadicalInverse(index, static_cast< float >(g_primes[i]));
		}
	}

	inline XMFLOAT2 Halton2D(size_t index) noexcept {
		const float x = RadicalInverse(index, 2.0f);
		const float y = RadicalInverse(index, 3.0f);
		return XMFLOAT2(x, y);
	}
	...
	inline XMFLOAT3 Halton3D(size_t index) noexcept {
		const float x = RadicalInverse(index, 2.0f);
		const float y = RadicalInverse(index, 3.0f);
		const float z = RadicalInverse(index, 5.0f);
	...
	inline XMFLOAT4 Halton4D(size_t index) noexcept {
		...
		const float y = RadicalInverse(index, 3.0f);
		const float z = RadicalInverse(index, 5.0f);
		const float w = RadicalInverse(index, 7.0f);
		return XMFLOAT4(x, y, z, w);
	}

	inline void Hammersley(size_t index, float *sample, size_t nb_dims, size_t nb_samples) noexcept {
		Assert(index < nb_samples);
		Assert(nb_dims <= _countof(g_primes));
		
		*sample = index / float(nb_samples);
		++sample;
	...
	inline XMFLOAT2 Hammersley2D(size_t index, size_t nb_samples) noexcept {
		...
		const float x = index / float(nb_samples);
		const float y = RadicalInverse(index, 2.0f);
		return XMFLOAT2(x, y);
	}
	...
	inline XMFLOAT3 Hammersley3D(size_t index, size_t nb_samples) noexcept {
		...
		const float x = index / float(nb_samples);
		const float y = RadicalInverse(index, 2.0f);
		const float z = RadicalInverse(index, 3.0f);
		return XMFLOAT3(x, y, z);
	}
	...
	inline XMFLOAT4 Hammersley4D(size_t index, size_t nb_samples) noexcept {
		...
		const float y = RadicalInverse(index, 2.0f);
		const float z = RadicalInverse(index, 3.0f);
		const float w = RadicalInverse(index, 5.0f);
		return XMFLOAT4(x, y, z, w);
	}
