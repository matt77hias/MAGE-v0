...
namespace mage {
	...
	void ComboBoxAdd(HWND dialog, int id, const void *data, const wchar_t *desc) noexcept {
		...
		// Set the application-defined value associated with the specified list item in a combo box. 
	...
	void ComboBoxSelect(HWND dialog, int id, int index) noexcept {
		...
		// Place (posts) a message in the message queue associated with the thread that created
		// the specified window and returns without waiting for the thread to process the message.
		// 1. A handle to the window whose window procedure is to receive the message.
