...
namespace mage {
	...
	const DataT BinaryReader::Read() {
		...
		const U8 * const new_pos = m_pos + sizeof(DataT);
		ThrowIfFailed((m_pos <= new_pos),
			"%ls: overflow: no value found.",    GetFilename().c_str());
		ThrowIfFailed((new_pos <= m_end),
			"%ls: end of file: no value found.", GetFilename().c_str());
	...
	const DataT *BigEndianBinaryReader::ReadArray(size_t count) {
		const U8 * const old_pos = m_pos;
		const U8 * const new_pos = m_pos + sizeof(DataT) * count;
		ThrowIfFailed((m_pos <= new_pos),
			"%ls: overflow: no %llu values found.", 
			GetFilename().c_str(), static_cast< U64 >(count));
		ThrowIfFailed((new_pos <= m_end),
